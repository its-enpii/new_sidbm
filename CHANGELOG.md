...
- **RBAC Level Tombol (Action-Level) & Manajemen Role Tenant dari Panel Superadmin:**
  - **RBAC Level Tombol Frontend Menyeluruh:** Proteksi reaktif `useCan()` (`v-if="can('...')"`):
    - **Dashboard:** Tombol *Quick Actions* (Register Proposal: `loans.propose`, Jurnal Angsuran: `installments.record`, Jurnal Umum: `journals.create`, E-Budgeting: `budgeting.view`), "Buat jurnal" (`journals.create`), "Catat angsuran" (`installments.record`), dan "Lihat semua di Tahapan Perguliran" (`loans.view`).
    - **Pelaporan & Ekspor:** Komponen filter laporan (`ReportPeriodFilter.vue`) mempagari unduh PDF dan Excel dengan `reports.view`. Kartu bundle laporan (`AnnualPack.vue`) mempagari unduh Bundle ZIP, Excel Auditor, dan PDF LPJ dengan `reports.view`. Catatan CALK dipagari `reports.manage`. Seluruh 7 halaman laporan perguliran pinjaman dan simulasi kredit mempagari cetak PDF dengan `loans.view`.
    - **Akuntansi & Lending:** Tombol bulk reverse dan link edit transaksi jurnal dipagari `journals.create`. Tombol pengisian dan penyimpanan alokasi laba ditahan di tutup buku dipagari `period_close.manage`. Tombol "Riwayat Perubahan", "Kartu Pinjaman", dan "Simpan Pengurus" di detail pinjaman dipagari `loans.view`/`loans.manage`.
    - **Master Data, Tagihan, & Pengaturan:** Tombol "Edit Lembaga" dipagari `institutions.manage`, form unggah/hapus foto KTP anggota dipagari `members.manage`, tombol pembayaran invoice dipagari `billing.pay`, tombol manajemen instance WhatsApp Hub dipagari `settings.manage`, serta tombol simpan pengaturan situs publik dipagari `website.manage`.
    - **Manajemen Akses & Pengguna:** Tombol aksi pada halaman `Access/Users/Index.vue` (`users.manage`), `Access/Roles/Index.vue` (`roles.manage`), dan tombol-tombol konfigurasi di `Settings/Index.vue` (`settings.manage`).
