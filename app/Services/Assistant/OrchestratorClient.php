...
final class OrchestratorClient
{
    ...
    {
        return AssistantSettingsResolver::orchestratorBaseUrl() !== ''
    ...
    public function issueSessionToken(
        ...
        $base = rtrim((string) ($adapterBaseUrl
            ?: AssistantSettingsResolver::adapterBaseUrl()
            ?: config('app.url')), '/');
        ...
        $response = Http::timeout((int) config('assistant.timeout', 10))
            ->withToken(AssistantSettingsResolver::sharedSecret())
            ->acceptJson()
            ->post(AssistantSettingsResolver::orchestratorBaseUrl().'/api/v1/sessions', $payload);
    ...
    {
        return AssistantSettingsResolver::orchestratorPublicUrl();
    }

    /**
     * Stream chat SSE from orchestrator. Yields ['event' => string, 'data' => string].
     *
     * @return \Generator<int, array{event: string, data: string}>
     */
    public function streamChat(string $message, ?string $conversationId = null, ?string $personaSlug = null): \Generator
    {
        if (! $this->isConfigured()) {
            throw new RuntimeException('Assistant orchestrator is not configured.');
        }
        ...
        $session = $this->issueSessionToken(
            externalUserId: 'admin-test-'.bin2hex(random_bytes(4)),
        ...
        $response = Http::timeout(120)
            ->withToken($session['session_token'])
            ->acceptJson()
            ->withOptions(['stream' => true])
            ->post(AssistantSettingsResolver::orchestratorBaseUrl().'/api/v1/chat', $body);

        if (! $response->successful()) {
            throw new RuntimeException('Chat HTTP '.$response->status().': '.$response->body());
        }
        ...
        foreach ($response->toStream() as $chunk) {
            ...
            while (($pos = strpos($buffer, "\n\n")) !== false) {
                ...
                $parsed = ['event' => 'message', 'data' => $event];
                $lines = explode("\n", $event);
                foreach ($lines as $line) {
                    if (str_starts_with($line, 'event:')) {
                        $parsed['event'] = trim(substr($line, 6));
                    } elseif (str_starts_with($line, 'data:')) {
                        $parsed['data'] = trim(substr($line, 5));
                    }
                }
                yield $parsed;
            }
