...
const pdfUrl = computed(() => {
    ...
    return `/lending/reports/lpp-kelompok/pdf?${q.toString()}`;
});
</script>

<template>
    <Head title="LPP Rincian Kelompok" />
    <AuthenticatedLayout>
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.18em] text-on-surface-variant">Laporan Piutang</p>
                    <h1 class="mt-1 text-2xl font-bold text-primary">LPP Rincian per Kelompok</h1>
                    <p class="mt-1 text-sm text-on-surface-variant">
                        Daftar perkembangan piutang rincian per kelompok pemanfaat dalam setiap desa
                    </p>
                </div>
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead class="bg-surface-variant/40 text-on-surface font-semibold">
                            <tr class="border-b border-outline-variant/30 text-center">
                                <th rowspan="2" class="p-3 text-left">Kelompok / Loan ID</th>
                        ...
                        <tbody class="divide-y divide-outline-variant/20">
                            <template v-for="v in prod.villages" :key="v.village_name">
                                <tr class="bg-surface-variant/30 font-bold text-on-surface">
                                    <td colspan="16" class="p-2.5 px-3">DESA: {{ v.village_name.toUpperCase() }}</td>
                                </tr>
                                <tr v-for="loan in v.loans" :key="loan.loan_id" class="hover:bg-surface-variant/10">
                                    <td class="p-2.5 px-3 font-medium text-on-surface">
                                        {{ loan.group_name }} <span class="text-on-surface-variant text-[11px]">(#{{ loan.loan_id }})</span>
                                    </td>
                                    <td class="p-2.5 text-center text-on-surface-variant">{{ loan.disbursed_at || '-' }}</td>
                                    <td class="p-2.5 text-center">{{ loan.pemanfaat_count }}</td>
                                    ...
                                    <td class="p-2.5 text-right border-l border-outline-variant/20">{{ formatMoney(loan.real_lalu_pokok) }}</td>
                                    <td class="p-2.5 text-right">{{ formatMoney(loan.real_lalu_jasa) }}</td>
                                    <td class="p-2.5 text-right border-l border-outline-variant/20">{{ formatMoney(loan.real_ini_pokok) }}</td>
                                    <td class="p-2.5 text-right">{{ formatMoney(loan.real_ini_jasa) }}</td>
                                    <td class="p-2.5 text-right border-l border-outline-variant/20 font-semibold">{{ formatMoney(loan.real_kumulatif_pokok) }}</td>
                                    <td class="p-2.5 text-right font-semibold">{{ formatMoney(loan.real_kumulatif_jasa) }}</td>
                                    <td class="p-2.5 text-right border-l border-outline-variant/20 font-semibold text-primary">{{ formatMoney(loan.saldo_pokok) }}</td>
                                    <td class="p-2.5 text-right font-semibold text-primary">{{ formatMoney(loan.saldo_jasa) }}</td>
                                    <td class="p-2.5 text-right border-l border-outline-variant/20 font-semibold" :class="loan.tunggakan_pokok > 0 ? 'text-error' : ''">
