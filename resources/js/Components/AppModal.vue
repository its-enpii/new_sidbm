...
<template>
    <Teleport to="body">
        <Transition name="modal">
            <div v-if="model" class="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-primary/45 p-4" @click.self="close">
                <section ref="panel" role="dialog" aria-modal="true" :aria-labelledby="titleId" tabindex="-1" :class="['flex max-h-[calc(100vh-2rem)] w-full flex-col rounded-2xl bg-surface-container-lowest shadow-2xl focus:outline-none', sizeClass[size] ?? sizeClass.md]" @keydown="onKeydown">
                    <header class="flex shrink-0 items-center justify-between gap-4 border-b border-outline-variant px-5 py-4 sm:px-6">
                        ...
                        <button v-if="closeable" type="button" class="grid size-10 shrink-0 place-items-center rounded-full text-on-surface-variant hover:bg-surface-container-low hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary-container/30" aria-label="Tutup modal" @click="close"><AppIcon name="close" /></button>
                    </header>
...
.modal-leave-to section { opacity: 0; transform: translateY(0.5rem) scale(0.98); }

@media (prefers-reduced-motion: reduce) {
    .modal-enter-active,
    .modal-leave-active,
    .modal-enter-active section,
    .modal-leave-active section { transition: none; }
}
</style>
