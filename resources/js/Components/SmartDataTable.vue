...
<template>
    <div class="space-y-3 sm:space-y-4">
        <div class="flex flex-col gap-3 sm:gap-4 border-b border-outline-variant pb-4 sm:pb-5 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-center gap-2 sm:gap-3"><SmartSelect id="smart-table-per-page" label="Per halaman" hide-label :model-value="Number(perPage)" :options="perPageOptions" :disabled="processing" @update:model-value="changePerPage"/><slot name="toolbar" /></div>
        ...
        <div class="relative overflow-x-auto">
            ...
            <table class="w-full text-left"><thead class="bg-surface-container-low text-sm"><tr><th v-for="column in columns" :key="column.key" class="px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 lg:py-3.5" :class="column.class"><button v-if="column.sortable" type="button" class="inline-flex items-center gap-1 font-bold text-primary" @click="sortBy(column)">{{ column.label }}<AppIcon :name="sort === column.key ? direction === 'asc' ? 'arrow_upward' : 'arrow_downward' : 'unfold_more'" class="text-lg text-outline" /></button><span v-else>{{ column.label }}</span></th><th v-if="$slots.actions" class="px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 lg:py-3.5 text-right">Aksi</th></tr></thead><tbody><tr v-for="(row, index) in rows" :key="row.row_id || row.id || index" class="border-t border-outline-variant"><td v-for="column in columns" :key="column.key" class="px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 lg:py-3.5" :class="column.class"><slot :name="`cell-${column.key}`" :row="row" :value="row[column.key]">{{ row[column.key] ?? '—' }}</slot></td><td v-if="$slots.actions" class="px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 lg:py-3.5 text-right"><slot name="actions" :row="row" /></td></tr></tbody></table>
            <div v-if="!rows.length && !processing" class="p-4 sm:p-6"><AppEmptyState icon="database" :title="emptyTitle" :description="emptyDescription" /></div>
        </div>
        <div class="flex flex-col gap-3 text-xs sm:text-sm text-on-surface-variant sm:flex-row sm:items-center sm:justify-between"><p>Menampilkan {{ pagination.from || 0 }}–{{ pagination.to || 0 }} dari {{ pagination.total || 0 }} data</p><nav v-if="lastPage > 1" class="flex items-center gap-1" aria-label="Pagination"><AppButton variant="ghost" size="compact" :disabled="currentPage === 1 || processing" aria-label="Halaman sebelumnya" @click="goTo(currentPage - 1)"><AppIcon name="chevron_left" /></AppButton><AppButton v-for="page in pages" :key="page" size="compact" :variant="page === currentPage ? 'primary' : 'ghost'" :disabled="processing" @click="goTo(page)">{{ page }}</AppButton><AppButton variant="ghost" size="compact" :disabled="currentPage === lastPage || processing" aria-label="Halaman berikutnya" @click="goTo(currentPage + 1)"><AppIcon name="chevron_right" /></AppButton></nav></div>
    </div>
</template>
